class e {
  ...
  getFormData(e) {
    ...
    for (let r = 0; r < i.length; r++) {
      let t = i[r],
        ...
        n = i[r]
      if (
        (l || (l = 'SELECT' === t.tagName ? 'select' : 'TEXTAREA' === t.tagName ? 'textarea' : ''),
        'INPUT' === t.tagName)
      )
        'radio' === l || 'checkbox' === l
        ...
        for (let i = 0; i < e.querySelectorAll('select').length; i++) {
          ...
          if (
            (!l.getAttribute('newbs') && l.setAttribute('newbs', i), l.getAttribute('newbs') == s)
          ) {
  ...
  previewBox() {
    ...
    if (e)
      return (
        e.querySelector('iframe') && e.querySelector('iframe').remove(),
        { close: e.querySelector('.previewClose'), previewBody: e.querySelector('.previewBody') }
      )
