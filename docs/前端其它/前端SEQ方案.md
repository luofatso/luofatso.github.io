...
SEO(Search Engine Optimization)搜索引擎优化;就是指按照搜索引擎的算法(爬虫--'蜘蛛')，提升你的文章(网站)在搜索引擎中的自然排名。
...
SEO还是有必要做的,只不过在百度这个平台会越来越艰难,因为现在流量被多个平台分割得很严重。

但是你可以把SEO这套模式直接复制到其他平台啊，例如一些自媒体平台，微信、知乎、头条等等。因为不管是哪个平台，只要发生搜索行为，就一定涉及到排序问题，那么SEO这套模式还是能发挥出价值啊！
...
- 语义化书写HTML代码，符合W3C标准
  ...
  尽量让代码语义化，在适当的位置使用适当的标签，用正确的标签做正确的事。让阅读源码者和“蜘蛛”都一目了然。比如：h1-h6 是用于标题类的，标签是用来设置页面主导航，列表形式的代码使用ul或ol，重要的文字使用strong等。 
  
- a标签

  页内链接需要加 “title” 属性加以说明，让访客和 “蜘蛛” 知道。而外部链接，则需要加 rel="nofollow" 属性, 告诉 “蜘蛛” 不要爬，因为一旦“蜘蛛”爬了外部链接之后，就不会再回来了。
  ```
   <a href="https://www.google.com" title="google搜索" rel="nofollow"></a>
  ```  
...
- strong标签
  ...
  - strong标签 ：需要强调时使用。strong标签在搜索引擎中能够得到高度的重视，它能突出关键词，表现重要的内容，strong标签强调效果仅次于h标签；

  - b标签：只是用于显示效果时使用，在SEO中不会起任何效果。 
  
- 重要内容不要用JS输出，因为“蜘蛛”不会读取JS里的内容，所以重要内容必须放在HTML里。 
- 尽量少使用iframe框架,因为“蜘蛛”不会读取其中的内容。 
