...
如果只需要简单的纯文本输入（如评论框、搜索框），且希望保持默认的表单提交行为、移动端友好性，原生 `textarea` 肯定是更简单可靠的选择。

但是有时候需求动机是突破纯文本限制，实现富文本、动态布局或复杂交互的时候，使用 `div` 替代 `textarea` 则更加灵活。

## 效果

案例为用户名校验，列表中如果有不符合规定的用户名，输入框将会变红，并调整到第一个错误的用户名的位置。

<script setup lang='ts'>
  import VueEdit from './components/VueEdit.vue'
</script>
...
<template>
  <div class="in-center">
    <el-input
      ...
      resize="none"
    />
    <div ref="editDivRef" class="in-center-edit" @input="handleInput" v-html="formattedText"></div>
